...
void Field::EnterPlayer(ClientSessionRef session, int32 selectedCharacterID)
{
	//플레이어를 입장시킨다.
...
void Field::AddGameObject(GameObjectRef object, ClientSessionRef execptSession)
{
...
void Field::BroadCast(SendBufferRef packet, ClientSessionRef execptSession)
{
	for (auto& player : _playerList)
	{
		ClientSessionRef session = player.second->GetSession();
